...
async function loadModel() {
  ...
  try {
    ...
    // ── Tự động đọc nhãn thật từ model ──
    modelClassNames = model.getClassLabels ? model.getClassLabels() : [];
    if (!modelClassNames.length && model.model) {
      // Fallback: thử lấy từ metadata
      try {
        const meta = await fetch(metaURL).then(r => r.json());
        modelClassNames = meta.labels || [];
      } catch (_) {}
    }

    // In ra console để người dùng biết nhãn thật
    console.log('📋 Nhãn mô hình thật:', modelClassNames);
    console.log('📌 CLASS_INDEX_MAP hiện tại:', CLASS_INDEX_MAP);

    // Tự động thêm nhãn thật vào LABEL_MAP nếu chưa có
    modelClassNames.forEach((name, idx) => {
      const key = name.toLowerCase().trim();
      if (!LABEL_MAP[key] && CLASS_INDEX_MAP[idx]) {
        LABEL_MAP[key] = CLASS_INDEX_MAP[idx];
        console.log(`🔗 Tự động map: "${name}" → ${CLASS_INDEX_MAP[idx]}`);
      }
    });

    // Hiển thị nhãn thật lên giao diện (toast thông báo)
    if (modelClassNames.length) {
      showToast(`✅ Model tải xong! ${modelClassNames.length} nhãn: ${modelClassNames.join(', ')}`, 'success');
    } else {
...
function renderConfBars(preds) {
  ...
  // Kiểm tra trùng lặp sau khi resolve (debug helper)
  const resolvedKeys = sorted.map(p => resolveLabel(p.className));
  const uniqueKeys = [...new Set(resolvedKeys)];
  if (uniqueKeys.length < sorted.length) {
    console.warn('⚠ Phát hiện nhãn bị map trùng! Hãy cập nhật CLASS_INDEX_MAP hoặc LABEL_MAP.');
    console.table(sorted.map((p, i) => ({ raw: p.className, resolved: resolvedKeys[i], prob: (p.probability*100).toFixed(1)+'%' })));
  }
  ...
  container.innerHTML = sorted.map((p, i) => {
    ...
    // Hiển thị tên nhãn thật từ model (p.className), kèm tên đã map
    const displayName = wt ? wt.name : p.className;
    const rawName = (modelClassNames.length && modelClassNames[sorted.indexOf(p)] !== undefined)
      ? '' : ` <span style="opacity:0.5;font-size:0.65rem">(${p.className})</span>`;
    return `
      <div class="conf-bar-wrap">
        <div class="conf-bar-label">
          <span>${wt ? wt.icon : '?'} ${displayName}${rawName}</span>
          <span>${pct}%</span>
        </div>
...
// ─── LABEL DETECTION (tự động phát hiện nhãn thật từ model) ──
// Mảng này sẽ được điền tự động khi model tải xong
let modelClassNames = []; // Ví dụ: ['Thực phẩm thừa', 'Giấy - bìa carton', ...]

// Thứ tự này PHẢI khớp với thứ tự class trong Teachable Machine của bạn (0, 1, 2, 3)
// Mở console (F12) sau khi bật camera để xem "📋 Nhãn mô hình thật:" và điều chỉnh lại
const CLASS_INDEX_MAP = [
  'thuc-pham',           // Class 0
  'giay-carton',         // Class 1
  'chai-nhua-thuy-tinh', // Class 2
  'tui-nilon-ong-hut'    // Class 3
];

function resolveLabel(rawLabel) {
  const lower = rawLabel.toLowerCase().trim();

  // 1. Tra LABEL_MAP chính xác trước (exact match)
  if (LABEL_MAP[lower]) return LABEL_MAP[lower];

  // 2. Tìm theo index trong modelClassNames (an toàn nhất)
  const idx = modelClassNames.findIndex(n => n.toLowerCase().trim() === lower);
  if (idx !== -1 && CLASS_INDEX_MAP[idx]) {
    console.log(`✅ Nhãn "${rawLabel}" → index ${idx} → ${CLASS_INDEX_MAP[idx]}`);
    return CLASS_INDEX_MAP[idx];
  }

  // 3. Partial match trên LABEL_MAP (tìm key chứa nhãn)
  for (const [mapKey, typeKey] of Object.entries(LABEL_MAP)) {
    if (lower.includes(mapKey) || mapKey.includes(lower)) return typeKey;
  }

  // 4. KHÔNG dùng fuzzy keyword nữa vì gây lặp — chỉ log để người dùng tự thêm map
  console.warn(`⚠ Không nhận diện được nhãn: "${rawLabel}". Hãy thêm vào LABEL_MAP!`);
  showToast(`⚠ Nhãn lạ: "${rawLabel}" – xem console để cập nhật`, 'warning');
  return CLASS_INDEX_MAP[0]; // fallback class đầu tiên
}
