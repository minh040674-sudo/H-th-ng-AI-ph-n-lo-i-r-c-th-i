...
const WASTE_TYPES = {
  'huu-co': {
    ...
    color: '#2dff6e',
    ecoMsg: '🌱 Rác hữu cơ có thể ủ phân compost, giúp cải tạo đất và giảm khí thải CH₄ từ bãi rác!',
    keywords: ['thức ăn', 'thực phẩm', 'lá cây', 'organic', 'huu co', 'hữu cơ'],
    threshold: 20,
    warningMsg: '⚠ Lượng rác hữu cơ đang nhiều. Cân nhắc lắp đặt thùng ủ compost tại chỗ!'
  },
  'tai-che': {
    name: 'Rác Tái Chế',
    icon: '🔵',
    ...
    led: 'blue',
    bin: '🟦',
    binLabel: 'Thùng Tái Chế (Xanh dương)',
    color: '#3da8ff',
    ecoMsg: '♻ Tái chế 1 tấn giấy có thể cứu 17 cây xanh và tiết kiệm 26.000 lít nước!',
    keywords: ['nhựa', 'plastic', 'chai', 'lon', 'giấy', 'recycling', 'tai che', 'tái chế'],
    threshold: 25,
    warningMsg: '⚠ Lượng nhựa tái chế rất cao! Cần thêm thùng tái chế lớn hơn hoặc liên hệ đơn vị thu mua.'
  },
  'nguy-hai': {
    name: 'Rác Nguy Hại',
    icon: '🔴',
    ...
    led: 'red',
    bin: '🟥',
    binLabel: 'Thùng Nguy Hại (Đỏ)',
    color: '#ff4d6a',
    ecoMsg: '⚠ Rác nguy hại KHÔNG được đổ xuống cống hay bỏ lẫn rác thường! Liên hệ đơn vị xử lý đặc biệt.',
    keywords: ['pin', 'bóng đèn', 'hóa chất', 'thuốc', 'nguy hại', 'hazardous'],
    threshold: 5,
    warningMsg: '🚨 KHẨN CẤP: Lượng rác nguy hại vượt ngưỡng! Cần liên hệ đơn vị xử lý chuyên dụng ngay!'
  },
  'vo-co': {
    name: 'Rác Vô Cơ',
    icon: '⚫',
    ...
    led: 'gray',
    bin: '⬛',
    binLabel: 'Thùng Rác Thường (Đen)',
    color: '#9ab09c',
    ecoMsg: '💡 Hãy cố gắng giảm thiểu rác vô cơ bằng cách lựa chọn sản phẩm ít bao bì hơn.',
    keywords: ['ni lông', 'xốp', 'vô cơ', 'inorganic', 'vo co'],
    threshold: 30,
    warningMsg: '⚠ Quá nhiều rác vô cơ! Hãy xem xét lại thói quen tiêu dùng và sử dụng đồ tái sử dụng.'
  }
};

// Alias mapping từ nhãn Teachable Machine → key của WASTE_TYPES
// !! Thay đổi các nhãn này để khớp với nhãn trong mô hình của bạn !!
const LABEL_MAP = {
  'organic':      'huu-co',
  'recyclable':   'tai-che',
  'hazardous':    'nguy-hai',
  ...
  'nguy hại':     'nguy-hai',
  'vô cơ':        'vo-co',
  // Thêm nhãn thực tế từ mô hình của bạn vào đây
};
...
function getAutoRecommendation(counts, total) {
  ...
  const recycled = (counts['tai-che'] || 0);
  const organic  = (counts['huu-co'] || 0);
  const hazardous = (counts['nguy-hai'] || 0);

  if (hazardous >= 5) return 'Ưu tiên xử lý ngay rác nguy hại theo quy định. Liên hệ đơn vị có chức năng xử lý đặc biệt!';
  if (recycled > total * 0.5) return 'Tuyệt vời! Bạn đang tái chế rất tốt. Tiếp tục duy trì và khuyến khích người xung quanh!';
  if (organic > total * 0.4) return 'Lượng rác hữu cơ nhiều – hãy xem xét lắp đặt thùng ủ phân compost để tái sử dụng tại chỗ!';
  return 'Hãy chú ý phân loại rác đúng thùng mỗi ngày. Chỉ cần 5 phút phân loại có thể giúp giảm 80% rác vào bãi chôn lấp!';
}
